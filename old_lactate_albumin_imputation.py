...
def inspect_y_trans(d: Dict, hist_args={"bins": 20, "alpha": 0.5}):
    """Compare original y (albumin or lactate) and its transformation."""
    fig, ax = plt.subplots(1, 2, figsize=(8, 3))
    ax = ax.ravel()

    for i, fold in enumerate(("train", "test")):
        ax[i].hist(d["pp"][fold].y_train, label="original", **hist_args)
        ax[i].hist(d["pp"][fold].y_train_trans, label="transformed", **hist_args)
        ax[i].set(title=fold)
        ax[i].legend()
...
def compare_folds(d: Dict):
    ...
    for fold in ["train", "test"]:
        ...
        for metric in (
            ...
            print(f"{metric.__doc__.splitlines()[0]} =", f"{metric(y_trans, y0_pred)}")
        ...
        inv_samples = d["trans"].inverse_transform(samples.flatten().reshape(-1, 1))
...
lac["pp"]["train"].y_train_trans = (
    lac["trans"].fit_transform(lac["pp"]["train"].y_train.reshape(-1, 1)).flatten()
)
lac["pp"]["test"].y_train_trans = (
    lac["trans"].transform(lac["pp"]["test"].y_train.reshape(-1, 1)).flatten()
)
...
for i in pb(range(lac["pp"]["train"].n_imp_dfs), prefix="GAM fit"):
    ...
    lac_gams.append(
        LinearGAM(
            ...
            + s(6, lam=500)  # wcc
            + s(7, lam=600)  # sbp
            + f(10, coding="dummy")  # gcs
            + f(11, coding="dummy")  # asa
            + te(  # urgency
                12,
                8,  # indication & ct
                lam=(2, 0.2),
...
subplot_labels = {
    7: {"lines": ["No CT", "CT"], "title": "Indication"},
    8: {"lines": ["Normal ECG", "Arrhythmia"], "title": "Heart rate"},
}

for i, term in enumerate(terms):
    if term["term_type"] != "tensor_term":
        XX = lac["gam"].generate_X_grid(term=i)
        pdep, confi = lac["gam"].partial_dependence(term=i, quantiles=cis)
        ...
        Z, confi = lac["gam"].partial_dependence(
        ...
        if i in (7, 8):
            ...
            ax3d.set_xlabel(
                lac["pp"]["train"].X["train"][0].columns[term["terms"][0]["feature"]]
            )
            ax3d.set_ylabel(
                lac["pp"]["train"].X["train"][0].columns[term["terms"][1]["feature"]]
            )
            plt.show()
...
Z, confi = lac["gam"].partial_dependence(term=i, X=XX, quantiles=cis, meshgrid=True)
...
alb = {
    "pp": {},
    "trans": GammaTransformer(op["winsor_thresholds"]["S03PreOpLowestAlbumin"]),
}
...
for fold in ("train", "test"):
    ...
    alb["pp"][fold].y_train_trans = alb["trans"].transform(alb["pp"][fold].y_train)
...
for i in pb(range(alb["pp"]["train"].n_imp_dfs), prefix="GAM fit"):
    ...
    alb_gams.append(
        GammaGAM(
            ...
            + s(7, lam=400)  # wcc
            + s(8, lam=300)  # sbp
            + f(12, coding="dummy")  # gcs
            + f(16, coding="dummy")  # asa
            + f(17, coding="dummy")  # perit soil
            + te(  # urgency
                18,
                10,  # indication & ct
                lam=(2, 1.0),
...
subplot_labels = {
    11: {"lines": ["No CT", "CT"], "title": "Indication"},
    12: {"lines": ["Normal ECG", "Arrhythmia"], "title": "Heart rate"},
}
...
for i, term in enumerate(terms):
    if term["term_type"] != "tensor_term":
        ...
        Z, confi = alb["gam"].partial_dependence(
            term=i, X=XX, quantiles=cis, meshgrid=True
        )
...
Z, confi = alb["gam"].partial_dependence(term=i, X=XX, quantiles=cis, meshgrid=True)
...
for d, name in ((lac, "S03PreOpArterialBloodLactate"), (alb, "S03PreOpLowestAlbumin")):
    for fold in ("train", "test"):
        for i in pb(range(d["pp"][fold].n_imp_dfs), prefix=f"{name} {fold}"):
            samples = quick_sample(
                d["gam"], d["pp"][fold].X["missing"][i], RANDOM_SEED, n_draws=1
            ).flatten()

            samples = d["trans"].inverse_transform(samples.reshape(-1, 1)).flatten()

            op[fold]["imp_all"][i].loc[
                op[fold]["imp_all"][i][name].isnull(), name
            ] = samples
