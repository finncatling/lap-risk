...
from nelarisk.helpers import (load_object, save_object,
                              check_system_resources)
from nelarisk.gam import combine_mi_gams, quick_sample
from nelarisk.evaluate import evaluate_predictions
from nelarisk.constants import (N_GAM_CONFIDENCE_INTERVALS,
...
# for i in pb(range(2), prefix='quick GAM fit'):
    gams.append(LogisticGAM(
        s(0, lam=200) +  #age
        s(2, lam=220) +  #sodium
        s(3, lam=300) +  #potassium
        s(5, lam=50) +   #wcc
        s(7, lam=300) +  #sbp
        s(8, lam=150, n_splines=13) +  #gcs
        s(19, lam=150) + #lactate
        s(20, lam=150) +  #albumin
        f(12, coding='dummy', lam=50) +  #asa
        f(21, coding='dummy', lam=200) +  #lactate missing
        f(22, coding='dummy', lam=200) +  #albumin missing
...
#            dtype=('categorical', 'categorical')) +
        te(15, 10,  # malig & ct
           lam=(200, 200),
           n_splines=(
               len(op['multi_cat_levels']['S03DiagnosedMalignancy']), 2),
           spline_order=(0, 0),
           dtype=('categorical', 'categorical')) +
        te(16, 10,  # perit soil & ct
           lam=(400, 200),
...
#            dtype=('categorical', 'categorical')) +
        ...
        te(13, 14,  # cardiac & resp
           lam=150,
           n_splines=(len(op['multi_cat_levels']['S03CardiacSigns']),
...
subplot_labels = {11: {'lines': ['No CT', 'CT'],
                       'title': 'Pred. malignancy'},
#                   11: {'lines': ['No CT', 'CT'],
#                        'title': 'Op. severity'},
#                   14: {'lines': ['No CT', 'CT'],
#                        'title': 'NCEPOD urgency'},
                  12: {'lines': ['No CT', 'CT'],
                       'title': 'Pred. perit. soiling'},
                  13: {'lines': ['No CT', 'CT'],
                       'title': 'Indication'},
                  14: {'lines': ['Normal ECG', 'Arrhythmia'],
                       'title': 'Heart rate'}}
...
for i, term in enumerate(terms):
    if term['term_type'] != 'tensor_term':
        ...
        Z, confi = gam.partial_dependence(term=i, X=XX,
                                          quantiles=cis,
                                          meshgrid=True)
...
#             plt.show()
            ...
            ax3d.set_xlabel(op['train']['imp_all'][0].columns[
                term['terms'][0]['feature']])
            ax3d.set_ylabel(op['train']['imp_all'][0].columns[
...
for i in pb(range(len(op[EVAL_FOLD]['imp_all']))):
    imp_samples.append(quick_sample(gam,
...
for i, outcome in enumerate(('lived', 'died')):
    strat_samples = samples[:, np.where(op[EVAL_FOLD]['y'] == i)[0]
                           ].flatten()
...
aes = [
    GAMTerm('S01AgeOnArrival',
            'Age (years)',
            (0, 0)),
    GAMTerm('S03Sodium',
            'Sodium (mmol/L)',
            (1, 1)),
    GAMTerm('S03Potassium',
            'Potassium (mmol/L)',
            (1, 2)),
    GAMTerm('S03WhiteCellCount',
            r'White cell count ($\times$10${^9}$/L)',
            (1, 0)),
    GAMTerm ('S03SystolicBloodPressure', 
             'Systolic pressure (mmHg)',
             (0, 1)),
    GAMTerm('S03GlasgowComaScore',
            'Glasgow Coma Score',
            (2, 2),
            None
#             list(range(3, 16))
           ),
    GAMTerm('S03PreOpArterialBloodLactate',
            'Lactate (mmol/L)',
            (2, 0)),
    GAMTerm('S03PreOpLowestAlbumin',
            'Albumin (g/L)',
            (2, 1)),
    GAMTerm('S03ASAScore',
            'ASA physical status',
            (3, 0),
            list(range(1, 6))),
    GAMTerm('S03PreOpArterialBloodLactate_missing',
            'Lactate missing',
            (3, 1),
            ['No', 'Yes']),
    GAMTerm('S03PreOpLowestAlbumin_missing',
            'Albumin missing',
            (3, 2),
            ['No', 'Yes']),
    GAMTerm('S03DiagnosedMalignancy',
            'Malignancy',
            (4, 1),
            ['None', 'Primary\nonly', 'Nodal\nmets.', 'Distant\nmets.'],
            ['No CT', 'CT'],
            'upper left'),
    GAMTerm('S03Pred_Peritsoil',
            'Peritoneal soiling',
            (4, 0),
            ['None', 'Serous', 'Local\npus', 'Free pus /\nblood / faeces'],
            ['No CT', 'CT'],
    ...
    GAMTerm('S03Pulse',
            ...
            ['Sinus', 'Arrhythmia'],
            'lower right'),
    GAMTerm(('S03CardiacSigns', 'S03RespiratorySigns'),
            ...
            None,
            (45, 205)),
    GAMTerm(('S03SerumCreatinine', 'S03Urea'),
