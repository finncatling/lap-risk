...
from nelarisk.helpers import (split_into_folds, save_object,
                              winsorize)
...
def combine_categories(df: pd.DataFrame,
                       combine: Dict[str, Dict[float, float]]
                      ) -> pd.DataFrame:
...
def add_missingness_indicators(df: pd.DataFrame, cols: list = [
    'S03PreOpArterialBloodLactate', 'S03PreOpLowestAlbumin']
) -> pd.DataFrame:
    ...
    for c in cols:
        ...
        df.loc[df[c].isnull(), c_missing] = 1.
    return df
...
def make_single_indications_variable(df: pd.DataFrame,
    indications: Tuple[str] = indications) -> pd.DataFrame:
...
binary_vars = list(set(cat_vars) -
                   set(multi_cat_levels.keys()))



def label_encode(df: pd.DataFrame,
                 multi_cat_levels = multi_cat_levels) -> pd.DataFrame:
    ...
    for c, levels in multi_cat_levels.items():
        if c is not 'Indication':
            ...
            df[c] = [np.nan if x == 'S05Ind_Missing' else levels.index(x)
                     for x in df[c].values]
    return df
...
def preprocess_data(data: pd.DataFrame) -> (pd.DataFrame, np.ndarray,
    pd.DataFrame, np.ndarray, Dict[str, Tuple[float, float]]):
    """Split NELA data in train and test folds, and preprocess each."""
    df = data.copy()
    
    df = select_vars(df)
    df = combine_categories(
        df, combine={'S03ECG' : {1.0: 0.0, 4.0: 1.0, 8.0: 1.0}})
    ...
    X_train_df, winsor_thresholds = winsorize(
        X_train_df, cont_vars=cont_vars,
        include={'S01AgeOnArrival': [False, True],
...
(X_train_df, y_train, X_test_df, y_test,
     winsor_thresholds) = preprocess_data(df)
...
def drop_add_lactate_albumin_cols(
    ...
    drop: bool = True,
    lac_alb_cols: List[str] = ['S03PreOpArterialBloodLactate',
                               'S03PreOpLowestAlbumin',
...
def drop_multi_cat_cols(
    df: pd.DataFrame,
    multi_cat_cols: List[str] = list(multi_cat_levels.keys())
) -> pd.DataFrame:
...
def impute(
    ...
    cat_imp = CategoricalImputer(df_no_lac_alb, mice_imps,
                                 mice_cont_vars, binary_vars,
                                 multi_cat_vars, RANDOM_SEED)
    cat_imp.impute_all()
...
for fold_name, X_df, y in (('train', X_train_df, y_train),
                           ('test', X_test_df, y_test)):
    print(fold_name.upper())
    imp_results = impute(X_df, n_mice_imputations[fold_name])
    output[fold_name] = {'X_df': X_df,
                         'y': y,
                         'imp': {'MICEData': imp_results[0],
                                 'CategoricalImputer': imp_results[1]}}
...
def add_lactate_albumin_cols_multidf(
    ...
    lac_alb_df: pd.DataFrame
) -> List[pd.DataFrame]:
    """Adds back lactate- and albumin-related columns to a
        list of DataFrames."""
    dfs = copy.deepcopy(dfs)
    for i in range(len(dfs)):
        dfs[i] = drop_add_lactate_albumin_cols(
            dfs[i], lac_alb_df, drop=False)
    return dfs
...
for fold_name in ('train', 'test'):
    ...
    pkl_output[fold_name]['imp'] = pkl_output[
        fold_name]['imp']['CategoricalImputer'].imputed_dfs
...
pkl_output['missingness_vars'] = ['S03PreOpArterialBloodLactate_missing',
                                  'S03PreOpLowestAlbumin_missing']
...
for fold_name in ('train', 'test'):
    ...
    target_df = pd.DataFrame(
        np.zeros((output[fold_name]['X_df'].shape[0], 2)),
...
for c in output[fold_name]['imp']['MICEData'].data.columns:
    for fold_name in ('train', 'test'):
        output[fold_name]['imp'][
            'MICEData'].plot_imputed_hist(c,
                                          imp_hist_args=hist_args,
                                          obs_hist_args=hist_args,
                                          all_hist_args=hist_args)
    ...
    for i, v in enumerate(list(multi_cat_levels.keys())[1:]):
        ...
        try:
            for j in range(cat_imp.n_mice_dfs):
                ax[i].hist(cat_imp._v[v]['imp'][j], density=True, bins=bins,
                           alpha=1 / cat_imp.n_mice_dfs, color='black')
        except IndexError:
             pass
        ax[i].hist(cat_imp._v[v]['y_train'], density=True, bins=bins,
